...
public class LanguageControllerTest {
    ...
    private static final String END_POINT = "/api/language";
    ...
    public void updateLanguageTest() throws Exception {
        ...
        LanguageUpdateDto dto = LanguageUpdateDto.builder().name("renamed").build();
        String content = convertToNonNullValueJsonString(dto);

        this.mockMvc
            .perform(
                RestDocumentationRequestBuilders.put(END_POINT + "/{id}", language.getId())
                .content(content)
            ...
            .andDo(document(
                "language-update",
                pathParameters(parameterWithName("id").description("어학 ID")),
                requestFields(
                    fieldWithPath("name").description("어학 이름")
                ),
                responseFields(
